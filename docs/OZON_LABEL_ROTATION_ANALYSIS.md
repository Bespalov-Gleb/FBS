...
Использовать **поворот на уровне PIL** как основной и надёжный способ:

1. Убрать предварительный вызов `_rotate_pdf` в пути noscale.
2. Передавать в `_ozon_fbs_to_standard_label` `rotate=ozon_rot` (90 по умолчанию).
3. Внутри всегда поворачивать изображение PIL, когда `rotate` задан.
4. PIL `Image.rotate(angle, expand=True)` работает предсказуемо для любых изображений, полученных из pdf2image.

Это устраняет зависимость от pypdf и метаданных PDF и даёт единый, контролируемый путь поворота.
