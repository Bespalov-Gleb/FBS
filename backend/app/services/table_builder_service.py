...
def build_xlsx_from_paths(input_paths: list[Path]) -> tuple[bytes, str, dict[str, int]]:
    ...
    return buffer.getvalue(), filename, {"ozon": len(ozon_articles), "wb": len(wb_articles)}
...
def parse_ozon_articles(file_path: Path) -> list[str]:
    if _is_csv(file_path):
        matrix = _csv_matrix(file_path)
        if len(matrix) < 2:
            raise ValueError(f"Файл {file_path.name}: в CSV нет строк данных под заголовком.")
