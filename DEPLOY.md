...
### Ozon API: HTTP 404 при синхронизации
При синхронизации Ozon в логах Celery может появляться `HTTP 404 — 404 page not found` для `/v4/products/info/attributes` (получение размеров товаров). Синхронизация продолжается, размеры просто не подтягиваются.

**Временное отключение запроса размеров** (если 404 мешает):
```env
FBS_OZON_FETCH_SIZES=0
```
Добавьте в `.env` и перезапустите контейнеры.
