...
- `FBS_PRINT_AGENT_TIMEOUT` — таймаут печати в секундах (по умолчанию 90). Если SumatraPDF «зависает» — увеличьте.
...
**SumatraPDF таймаут:** если печать «зависает» — увеличьте `FBS_PRINT_AGENT_TIMEOUT` (по умолчанию 90 с). Проверьте драйвер принтера и его доступность.

**Повреждённая установка:** используйте только **портативную** версию (ZIP), не installer. Портативная — один exe, без libmupdf.dll.

**Искажённая печать (чёрные блоки, наложение штрихкодов):** на термо-принтерах этикеток `noscale` часто даёт артефакты. Агент автоматически использует `fit` вместо `noscale`. Убедитесь, что в настройках принтера Windows задан правильный размер этикетки (58×40 или 80×40 мм).
